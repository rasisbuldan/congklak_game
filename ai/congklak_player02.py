...
class CongklakPlayer02(CongklakPlayer):
    ...
    def cariCabang(self, node1, nomor):
        ...
        for i in range(len(nexts)):
            ...
            a.setFaktor([self.faktor_lanjut, self.faktor_ulang,
                         self.faktor_tabung, self.faktor_tembak, self.faktor_mati])
            self.resetFaktor()
    ...
    def main(self, papan):
        ...
        for i in range(len(frontier)):
            ...
            try:
                evalScore[parent[0]][parent[1]].append(
                    self.evalFunc(frontier[i][1], no))
            except:
                ...
                if score[1][i] != []:
                    if score[1][i] == score[0][0]:
                        if node[1][i][2] not in pilihan:
                            pilihan.append(
                                (node[1][i][1].getTabungan(self.nomor), node[1][i][2]))
                            print(
                                "ALTERNATIF", node[1][i][2], score[node[1][i][0][0]][node[1][i][0][1]])
...
class CongklakModelSim:
    ...
    def pemenang(self):
        tabung = [self.__lubang[0][self.N_LUBANG],
                  self.__lubang[1][self.N_LUBANG]]
